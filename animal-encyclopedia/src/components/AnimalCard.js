export function createAnimalCard(animal) {
  ...
  const habitat = animal.habitat || animal.characteristics?.habitat || 'Unknown';
  const diet    = animal.diet    || animal.characteristics?.diet    || 'Unknown';

  const card = document.createElement('div');
  card.className = 'animal-card';
  card.innerHTML = `
    <img src="${imgSrc}" alt="${animal.name}" class="animal-card__img"/>
